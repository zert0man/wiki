...
1. Добавьте папку установки Ruby в переменную среды `Path`.
	...
	```
1. Подключите коннектор MySQL **для XAMPP:** зайдите в `xampp\mysql\lib\`, скопируйте файл `libmysql.dll` в папку `C:\RailsInstaller\Ruby2.1.0\bin\`. 
1. Подключите коннектор MySQL **для виртуальной машины в Vagrant:** скачайте [архив с библиотекой коннектора MySQL на C](http://railsinstaller.org/ru-RUhttp://dev.mysql.com/downloads/connector/c/6.0.html) (желательно версию 6.0.2, самая новая не рекомендуется, `mysql-connector-c-noinstall-6.0.2-win32.zip`, если предлагают зарегистрироваться - в самом низу страницы скачивания есть строка `No thanks, just start my download.`), распакуйте содержимое в  `C:\temp\mysqlconnector\`, скопируйте файл `libmysql.dll` из подпапки `lib` в папку `C:\RailsInstaller\Ruby2.1.0\bin\`.
1. Выполните команду установки гема `mysql2` (в параметре команды укажите полный путь до mysql). Для XAMPP:
	
	```
		gem install mysql2 -- '--with-mysql-dir="C:\...\xampp\mysql"'
	```

	Для mesdt.box в Vagrant:

	```
		gem install mysql2 -- '--with-mysql-dir="C:\temp\mysqlconnector\"'
	```

	или другая папка, в которую вы распаковали коннектор.
...
1. Создайте приложение Ruby on Rails. Это делается командой (параметр `C:\my\path\to\ruby\lab\` - это имя каталога, в котором будет создано приложение).
	...
	Команда работает не очень быстро, потому что докачивает из интернета необходимые гемы. 
	Если при скачивании гемов ругается не SSL, откройте созданный системой файл `Gemfile` и отредактируйте первую строку:
	...
	При перезапуске команды `new` ответьте отрицательно на запрос о перезаписи файла `Gemfile`
	
	Перейдите в папку `C:\my\path\to\ruby\lab\`.
...
1. Запустите сервер Ruby командой 
	...
	По умолчанию сервер Ruby занимает порт 3000. Зайдите на `localhost:3000` и порадуйтесь, что сайт заработал. Все последующие изменения можно производить при включенном сервере, они будут подхватываться "на лету". Вам придется запустить еще одно окно консоли, чтобы не останавливать сервер.
...
1. Зайдите в файл конфигурации маршрутов, раскомментируйте и исправьте строчку с маршрутом до главной страницы. Подробно про маршруты читайте в [документации](http://rusrails.ru/rails-routing).
