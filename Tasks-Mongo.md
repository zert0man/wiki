...
### Датасет 2. Пьессы Шекспира.

Датасет получен с сайта [http://shakespeare.mit.edu](http://shakespeare.mit.edu) и содержит 36 пьесс Уильяма Шекспира.

[Ссылка для скаивания](https://yadi.sk/d/ZX6Z0n-FfJst9).

Каждый документ пьессы содержит в себе коллекцию актов, состощих из сцен, каждая сцена содержит коллекцию реплик, разбитых на предложения например:
...
1. Датасет 2. Найдите сцену с самым большим количеством реплик в сцене (надо указать название пьессы, акт и сцену)
2. Датасет 1. Покопаемся в переписке Shanna Husser и Eric Bass. Сколько писем каждый из них отправил другому?
1. Датасет 2. Количество актов и сцен в каждой пьессе
...
1. Датасет 2. Список персонажей для каждой пьессы, отсортированный по алфавиту.
...
1. Датасет 2. Какие персонажи встречаются больше чем в одной пьессе?
1. Датасет 1. В какой день недели отправлено максимальное количество писем?

1. * Датасет 2. Для каждой пьессы найдите персонажа, у которого самое большое количество реплик.
